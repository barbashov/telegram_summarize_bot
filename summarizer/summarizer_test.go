...
func TestClusterTopicsSanitizesAssignments(t *testing.T) {
	...
	sum := NewWithClient(client, "test-model")
...
func TestClusterTopicsRejectsInvalidJSON(t *testing.T) {
	sum := NewWithClient(&fakeChatClient{responses: []string{"not-json"}}, "test-model")
...
func TestSummarizeByTopicsPropagatesClientError(t *testing.T) {
	sum := NewWithClient(&fakeChatClient{err: errors.New("boom")}, "test-model")
