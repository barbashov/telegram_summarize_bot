...
- **`handlers/`** — Telegram update handling (polling, not webhooks), rate limiter, and group commands: `summarize` (with `s`/`sub` shorthands), reply-summarization (`summarize_reply.go`), scheduled summaries, and help. Handles `edited_message` (`edited.go`) by overwriting the stored text in the DB so later summaries reflect the edit. `Bot` struct owns the telego client, DB, summarizer, and rate limiter. Background goroutines handle message cleanup, rate-limit-entry cleanup, stats cache refresh, and scheduled summaries. Outgoing text is rendered via `telegramify` (Markdown → MarkdownV2); admin reports are sent as plain text.
